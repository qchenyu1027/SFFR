...
import thop  # for GFLOPS calculation
...
def test(data,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            # Run model
            t = time_synchronized()
            out, _, train_out = model(img_rgb, img_ir)  # inference and training outputs
        ...
        if plots and batch_i < 3:
            f1 = save_dir / f'test_batch{batch_i}_labels.jpg'  # labels
            Thread(target=plot_images, args=(img_rgb, targets, paths, f1, names), daemon=True).start()
            f2 = save_dir / f'test_batch{batch_i}_pred.jpg'  # rgb predictions
            Thread(target=plot_images, args=(img_rgb, output_to_target(out), paths, f2, names), daemon=True).start()

            # Add IR image prediction plotting
            f3 = save_dir / f'test_batch{batch_i}_ir_pred.jpg'  # ir predictions
            # Assuming you have access to the IR images as img_ir
            Thread(target=plot_images, args=(img_ir, output_to_target(out), paths, f3, names),
                   daemon=True).start()
    # 保存所有预测框结果，后续用于MR的计算
    if save_txt:
        temp = []
    ...
    # MR_partial * 100, MR_heavy * 100, recall_all * 100))
    ...
    # 计算时间（确保单位一致）
    # t0_ms = t0 / seen * 1E3  # 将 t0 转换为 ms
    # t1_ms = t1 / seen * 1E3  # 将 t1 转换为 ms
    # total_time_ms = t0_ms + t1_ms  # 总时间 (ms)
    #
    # # 计算 FPS
    # FPS = 1000 / total_time_ms  # 计算 FPS

    # 打印结果
    # print(
    #     f"Speed: {t0_ms:.1f}/{t1_ms:.1f}/{total_time_ms:.1f} ms inference/NMS/total per {imgsz}x{imgsz} image at batch-size {batch_size}")
    # print(f"FPS: {FPS:.3f}")
    # from thop import profile
    # flops, params = profile(model, inputs=(img_rgb, img_ir), verbose=False)
    # Gflops = flops / 1e9  # 转换成 GFLOPS
    # print(f"GFLOPS: {Gflops:.2f}, Params: {params:,}")
    # Print speeds
    ...
    if save_json and len(jdict):
        ...
        anno_json = '/home/ailab/qqqcy/DroneVehicle/annotations/instances_val2017.json'  # annotations json
        # anno_json = '/home/ailab/qqqcy/FLIR-align-3class/annotations/instances_val2017.json'  # annotations json
        # anno_json = '/media/ailab/EXTERNAL_USB/VEDAI1/fold01/annotations/instances_val2017.json'  # annotations json
        # anno_json = '/media/ailab/EXTERNAL_USB/seadrone/annotations/instances_val2017.json'  # annotations json
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--weights', nargs='+', type=str, default='/media/ailab/EXTERNAL_USB/latest/SeaDroneSee/exp3/weights/best.pt', help='model.pt path(s)')
    # parser.add_argument('--weights', nargs='+', type=str, default='/media/ailab/5385f1e6-0f46-4865-90c2-287b9a0f3c16/qcy/latest/flir/exp118/weights/best.pt', help='model.pt path(s)')
    parser.add_argument('--weights', nargs='+', type=str, default='/home/shen5/zhb/2025_5_9/baseline_DroneVechilex/weights/last.pt', help='model.pt path(s)')
    # parser.add_argument('--weights', nargs='+', type=str, default='/media/ailab/5385f1e6-0f46-4865-90c2-287b9a0f3c16/qcy/FLIR/exp25/weights/best.pt', help='model.pt path(s)')
    # parser.add_argument('--weights', nargs='+', type=str, default='/media/ailab/EXTERNAL_USB/icafusion/VEDAI/exp21/weights/best.pt', help='model.pt path(s)')
    # parser.add_argument('--weights', nargs='+', type=str, default='/media/ailab/5385f1e6-0f46-4865-90c2-287b9a0f3c16/qcy/sea/exp1+64/weights/best.pt', help='model.pt path(s)')
    ...
    # parser.add_argument('--data', type=str, default='data/multispectral/VEDAI.yaml', help='*.data path')
    # parser.add_argument('--data', type=str, default='data/multispectral/kaist.yaml', help='*.data path')
    # parser.add_argument('--data', type=str, default='data/multispectral/seadrone.yaml', help='data.yaml path')
    # parser.add_argument('--data', type=str, default='data/multispectral/Sea.yaml', help='data.yaml path')
    parser.add_argument('--batch-size', type=int, default=6, help='size of each image batch')
    # parser.add_argument('--img-size', type=int, default=[640,512], help='inference size (pixels)')
    ...
    # p = "/home/ailab/qqqcy/FLIR-align-3class/labels/test"
    # p = "/media/ailab/EXTERNAL_USB/VEDAI/fold01/labels/test"
    p = "/home/shen/cv_dataset/DroneVechile/labels/test"
    # p = "/media/ailab/5385f1e6-0f46-4865-90c2-287b9a0f3c16/qcy/SeeDroneSee/labels/val"
    # p = "/media/ailab/EXTERNAL_USB/KAIST/infrared/test"
    # p = " /home/ailab/FLIR-align-3class/visible/train"
    labels_list = os.listdir(p)
    labels_list.sort()
    if opt.data in ['./data/multispectral/FLIR-align-3class.yaml','data/multispectral/kaist.yaml','data/multispectral/Sea.yaml','data/multispectral/seadrone.yaml', './data/multispectral/FLIR-ADAS.yaml', './data/multispectral/VEDAI.yaml','./data/multispectral/DroneVehicle.yaml']:
        opt.verbose = True
