...
def detect(opt):
    ...
    for (path, img, im0s, vid_cap), (path_, img2, im0s_, vid_cap_) in zip(dataset, dataset2):
        ...
        pred = model([img.half(), img2.half()], augment=False)[0]

        # Apply NMS
        pred = non_max_suppression(pred, opt.conf_thres, opt.iou_thres, classes=opt.classes, agnostic=opt.agnostic_nms)
        t2 = time_synchronized()
        flop, params = profile(model, inputs=(img.half(), img2.half()))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='ICAFusion_FLIR.pt', help='model.pt path(s)')
    parser.add_argument('--source1', type=str, default='DOVOT/visible', help='source')  # file/folder, 0 for webcam
    parser.add_argument('--source2', type=str, default='DOVOT/infrared', help='source')  # file/folder, 0 for webcam
