...
def train_rgb_ir(hyp, opt, device, tb_writer=None):
    ...
    if pretrained:
        ...
        for key in list(state_dict.keys()):
            if int(key.split('.')[1]) < 10:
                new_state_dict[key] = state_dict[key]
                new_state_dict[key[:6] + str(int(key.split('.')[1]) + 10) + key[7:]] = state_dict[key]
            if int(key.split('.')[1]) > 9:
                new_state_dict[key[:6] + str(int(key.split('.')[1]) + 13) + key[8:]] = state_dict[key]
        new_state_dict = intersect_dicts(new_state_dict, model.state_dict(), exclude=exclude)
    ...
    # labels_path = data_dict['path'] + '/labels/val'
    ...
    # Process 0
    if rank in [-1, 0]:
        testloader, testdata = create_dataloader_rgb_ir(test_path_rgb, test_path_ir,imgsz_test, 8, gs, opt,
    ...
    # hyp['box'] *= (1.5) / nl  # scale to layers
    hyp['cls'] *= nc / 80. * 3. / nl  # scale to classes and layers
    # hyp['cls'] *= max(nc / 80., 1.0) * 3. / nl  # 确保类别少时权重不会太小
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if rank in [-1, 0]:
                ...
                # if ni:
                #     # 设置保存路径
                #     save_dir1 = r"/home/ailab/qqqcy/valbounding/train"
                #
                #     # 拼接文件路径
                #     f1 = os.path.join(save_dir1, f'train_batch{ni}_vis.jpg')
                #     f2 = os.path.join(save_dir1, f'train_batch{ni}_inf.jpg')
                #
                #     # 启动线程保存可视化图片
    ...
    if rank in [-1, 0]:
        ...
        # Test best.pt
        logger.info('%g epochs completed in %.3f hoursbest.\n' % (epoch - start_epoch + 1, (time.time() - t0) / 3600))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='ICAFusion_FLIR.pt', help='initial weights path')
    parser.add_argument('--cfg', type=str, default='models/yolov5l-CLLADetect.yaml', help='model.yaml path')
    ...
    # parser.add_argument('--resume', nargs='?', const=True, default='/media/ailab/EXTERNAL_USB/fusion/FLIR/full/weights/last.pt', help='resume most recent training')
    ...
    parser.add_argument('--device', default='0', help='cuda device, i.e 0 or 1')
    ...
    # parser.add_argument('--project', default='/media/ailab/EXTERNAL_USB/icafusion/kan', help='save to project/name')
    # parser.add_argument('--project', default='/media/ailab/EXTERNAL_USB/fusion/Sea',help='save to project/name')
    # parser.add_argument('--project', default='/media/ailab/EXTERNAL_USB/fusion/FLIR',help='save to project/name')
    parser.add_argument('--project', default='/home/shen5/zhb/2025_5_9',help='save to project/name')
    # parser.add_argument('--project', default='/media/ailab/EXTERNAL_USB', help='save to project/name')
    # parser.add_argument('--project', default='runs/train/VEDAI', help='save to project/name')
    # parser.add_argument('--project', default='/media/ailab/EXTERNAL_USB/latest/Ablation', help='save to project/name')
    # parser.add_argument('--project', default='/media/ailab/EXTERNAL_USB/new1', help='save to project/name')
    parser.add_argument('--entity', default=None, help='W&B entity')
    parser.add_argument('--name', default='FFKConv_multiscale_KAN_DroneVechile', help='save to project/name')
