...
from utils.general import make_divisible, check_file, set_logging,check_version,make_divisible1
from utils.torch_utils import time_synchronized, fuse_conv_and_bn, model_info, scale_img, initialize_weights, \
    select_device, copy_attr
from models.detectLSCD import *
from models.common import CBAM,Channel_Attention,Spatial_Attention
# from  nn.modules.head import *
...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=(),temperature_channels=4):  # detection layer
        ...
        # self.dysnakeconv = nn.ModuleList(DySnakeConv1(x, self.no * self.na)for x in ch)
        self.m = nn.ModuleList(nn.Conv2d(x, self.no * self.na, 1) for x in ch)  # output conv
        # self.n = nn.ModuleList(DecoupledHead(x, nc, anchors) for x in ch)  # output conv


    def forward(self, x, temp_input=None):
...
class Model(nn.Module):
    ...
    def forward_once(self, x, x2, profile=False):
        ...
        for m in self.model:
            ...
            if profile:
                o = thop.profile(m, inputs=(x,x2), verbose=False)[0] / 1E9 * 2 if thop else 0  # FLOPS
                t = time_synchronized()
                for _ in range(37):
                    _ = m(x)
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
                 BottleneckCSP,C3TR, C3STR, C3Ghost,ASPP, CBAM, nn.ConvTranspose2d,
                 C3, C3TR,ConvMix, CSPCM,SPDConv]:
            ...
            if m is Focus:
                ...
                args = [c1, c2, *args[1:]]
                if m in [BottleneckCSP, C3, C3TR,C3TR, C3STR, C3Ghost]:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='models/yolov5l-CLLADetect.yaml', help='model.yaml')
    ...
    img_size=640
    input_rgb =torch.Tensor(1,3,img_size,img_size)
    input_ir = torch.Tensor(1,3,img_size,img_size)
    input_ir=input_ir.to(device)
    input_rgb=input_rgb.to(device)
    # flop, params = thop.profile(model,inputs= (input_rgb, input_ir))
