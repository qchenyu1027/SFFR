...
class KAGNConvNDLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        rgb_fea_flat = torch.cat([
            self.base_conv[i](rgb_fea_flat)
            for i in range(self.groups)
        ], dim=1)
        # print(f"rgb_fea_flat",rgb_fea_flat.shape)

        ir_fea_flat = torch.cat([
            self.base_conv[i](ir_fea_flat)
            for i in range(self.groups)
        ], dim=1)
...
if __name__ == '__main__':
    ...
    block = KAGNConv2DLayer(128,128,3)
    input1 = torch.randn(1, 400, 128)  # 输入：BCHW格式
    input2 = torch.randn(1, 400, 128)  # 输入：BCHW格式
    ...
    print(f"Output size: {output[1].shape}")
    print("ok")